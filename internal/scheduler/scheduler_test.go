...
func TestSpreadPicksLeastUtilized(t *testing.T) {
	...
	busy := nodeWith(-10, cluster.NewPod("web"), cluster.NewPod("web"))
	got := Spread{}.Pick([]*cluster.Node{busy, empty}, cluster.NewPod("web"))
	if got != empty {
...
func TestBinPackPicksTightestFit(t *testing.T) {
	empty := cluster.NewNode(-10)
	busy := nodeWith(-10, cluster.NewPod("web"), cluster.NewPod("web"))
	got := BinPack{}.Pick([]*cluster.Node{busy, empty}, cluster.NewPod("web"))
	if got != busy {
...
func TestBinPackImageLocalityBreaksTies(t *testing.T) {
	a := nodeWith(-10, cluster.NewPod("api"))
	b := nodeWith(-10, cluster.NewPod("web"))
	got := BinPack{}.Pick([]*cluster.Node{a, b}, cluster.NewPod("web"))
	if got != b {
...
func TestVictimPolicies(t *testing.T) {
	low := nodeWith(-10, cluster.NewPod("web"))
	high := nodeWith(-10, cluster.NewPod("web"), cluster.NewPod("web"), cluster.NewPod("web"))
	st := &cluster.State{Nodes: []*cluster.Node{low, high}}

	if n, _ := Victim(BinPack{}, st); n != low {
		t.Fatalf("binpack should drain the emptiest node (consolidation)")
	}
	if n, _ := Victim(Spread{}, st); n != high {
