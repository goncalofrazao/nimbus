...
func (t *backoffTable) known(key string) bool { _, ok := t.m[key]; return ok }

// ready reports whether a restart is permitted for key at time now; if not, it
// returns the remaining wait.
func (t *backoffTable) ready(key string, now time.Time) (bool, time.Duration) {
	b := t.m[key]
	if b == nil || !now.Before(b.nextEligible) {
		return true, 0
	}
	return false, b.nextEligible.Sub(now)
}
...
func (t *backoffTable) forget(key string) { delete(t.m, key) }
